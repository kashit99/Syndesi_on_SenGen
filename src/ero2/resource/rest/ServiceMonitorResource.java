...
public class ServiceMonitorResource extends LocalResource {
  ...
  public void performPOST(POSTRequest request) {
    ...
    System.out.println("Heartbeat receving from " + serviceLocator
        + " with illuminance, temperature, humidity and actuation_state");
    ...
    if (ERO2REGISTRY.searchService(serviceLocator) != null) {
      ...
      int i = 1;
      String lum = "", temp = "", timestamp = "", hum = "", act=""; 
      ...
      while (tokenizer.hasMoreTokens()) {
		...
		case 3:
            ...
            break;
		case 4:
            status.setReading("actuation_state", token);
            act = token;
            System.out.println("act" + token);
            break;
        }
        i++;
      }
      ...
      ERO2REGISTRY.updateService(serviceLocator, lum, temp, hum, timestamp, act);

      ERO2REGISTRY.updateStatus(serviceLocator, status);
      System.out.println("Ero2 service updated " + serviceLocator + " "
          + ipaddr + " " + lum + " " + temp + " " + hum + " actuation_state=" + act + " at " + timestamp);
      ...
      this.updateSensors(serviceLocator,new Integer(lum),new Integer(temp), Integer.valueOf(hum.replaceAll("\\D+","")), timestamp, new Integer(act));
  ...
  private void updateSensors(String incoming_locator, int luminance, int temperature, int humidity, String timestamp, int actuation){

    System.out.println("Salut maman; ServiceLocator = "+ incoming_locator+ "; Luminance= "+ luminance+"; Temperature= "+ temperature + "; Humidity=" + humidity + "; ActuationState= "+ actuation +"; Timestamp= "+ timestamp);

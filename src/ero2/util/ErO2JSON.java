...
//      }
      ...
      String actuation_state = service.getActuation_state();
      String timestamp		= service.getTimestamp();
      //HACK TO GET THE FLOATING POINT FOR HUMIDITY AND TEMPERATURE
      System.out.println("tem = " + temperature + "  humi = " + humidity); //
      if (temperature!=null) {
...
//!!! END OF HACK
		...
		//luminance sensor
		  ...
		  nodeResourceJSONlumSen.put("actuation_state", actuation_state);
  ...
  public static String getTestbedSensorValuesJSONString(Hashtable<String, ErO2Service> serviceRegistry) {
    ...
    while (serviceKeys.hasMoreElements()) {
      ...
      String humidity		= ero2ser.getHumidity_value();
      String actuation_state = ero2ser.getActuation_state();
      //System.out.println("tem = " + temperature + "  humi = " + humidity);
      if (temperature!=null) {
     	 int temper = Integer.valueOf(temperature.replaceAll("\\D+",""));
     	 float temper2 = (float) temper / 100;
      ...
      nodeJSON.put("humidity", humidity);
      nodeJSON.put("actuation_state", actuation_state);
